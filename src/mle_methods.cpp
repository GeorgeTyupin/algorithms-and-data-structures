...
// ============ MLS для распределения Вейбулла (ТОЛЬКО полные данные) ============
// Использует взвешенный МНК через порядковые статистики (orderw)
MLEResult mls_weibull_complete(const std::vector<double>& data) {
    MLEResult result;
    int n = data.size();

    // Начальная оценка - используем MLE
    MLEResult initial_mle = mle_weibull_complete(data);
    result.initial_parameters = initial_mle.parameters;
    result.initial_log_likelihood = initial_mle.log_likelihood;
    ...
    // Заполнение матриц для взвешенного МНК
    // Для Вейбулла работаем в логарифмической шкале
    for (int i = 0; i < n; i++) {
        for (int j = i; j < n; j++) {
            double er_i, vrs;
            orderw(n, fcum[i], fcum[j], er_i, vrs);
            v(j, i) = vrs;
            v(i, j) = vrs;
        }
        double er, vrs;
        orderw(n, fcum[i], fcum[i], er, vrs);
        x(i, 0) = 1.0;      // столбец для log(λ)
        x(i, 1) = er;       // столбец для 1/k (математическое ожидание порядковой статистики)
        y(i, 0) = std::log(ycum[i]);  // логарифм наблюдаемых значений
    }
    ...
    // Результаты:
    // b(0,0) = log(λ), откуда λ = exp(b(0,0))
    // b(1,0) = 1/k, откуда k = 1/b(1,0)
    double log_lambda = b(0, 0);
    double inv_k = b(1, 0);
    double lambda = std::exp(log_lambda);
    double k = 1.0 / inv_k;

    result.parameters.push_back(lambda);  // λ (масштаб)
    result.parameters.push_back(k);       // k (форма)

    // Вычисление логарифма функции правдоподобия
    result.log_likelihood = 0.0;
    for (double x_val : data) {
        result.log_likelihood += std::log(k / lambda) + (k - 1) * std::log(x_val) -
                  std::pow(x_val / lambda, k);
    }

    // Ковариационная матрица параметров (из взвешенного МНК)
    // Нужно преобразовать ковариацию (log(λ), 1/k) в ковариацию (λ, k)
    // Используем дельта-метод для преобразования
    result.cov_size = 2;
    result.covariance = new double*[2];
    for (int i = 0; i < 2; i++) {
        result.covariance[i] = new double[2];
    ...
    // Приближенное преобразование через дельта-метод
    // Var(λ) ≈ λ² * Var(log(λ))
    // Var(k) ≈ k⁴ * Var(1/k)
    result.covariance[0][0] = lambda * lambda * db(0, 0);
    result.covariance[0][1] = lambda * k * k * db(0, 1);
    result.covariance[1][0] = lambda * k * k * db(1, 0);
    result.covariance[1][1] = k * k * k * k * db(1, 1);
    ...
    // Освобождение начальной ковариации MLE
    if (initial_mle.covariance != nullptr) {
        for (int i = 0; i < initial_mle.cov_size; i++) {
            delete[] initial_mle.covariance[i];
        }
        delete[] initial_mle.covariance;
    }
