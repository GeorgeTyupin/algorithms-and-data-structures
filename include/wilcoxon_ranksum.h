 ...
 *
 * Генерирует частоты (frequencies) для распределения Mann-Whitney U-статистики
 * для двух заданных размеров выборок.
 *
 * @param m Размер первой выборки
 * @param n Размер второй выборки
 * @param frqncy Вектор для хранения частот (размер m*n+1)
 * @param work Рабочий массив (размер >= (m*n+1)/2 + min(m,n))
 * @return 0 если успешно, код ошибки иначе:
 *         1 - некорректный размер выборки (< 1)
 *         2 - недостаточный размер массива frqncy
 *         3 - недостаточный размер рабочего массива work
 *
 * Примечание: После вызова frqncy[i] содержит частоту для U = i-1
 * (индексация с 0, поэтому frqncy[0] соответствует U=0)
 */
 ...
 *
 * @param u_statistic Значение U-статистики
 * @param m Размер первой выборки
 * @param n Размер второй выборки
 * @return p-value для двустороннего теста
 */
