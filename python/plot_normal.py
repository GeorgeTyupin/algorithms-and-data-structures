
"""
Визуализация результатов MLE/MLS для нормального распределения
"""

import numpy as np
import matplotlib.pyplot as plt
from scipy import stats
import sys

def read_results(filename):
    """Чтение результатов из файла"""
    params = {}
    data = []
    censored = []
    
    with open(filename, 'r') as f:
        section = None
        for line in f:
            line = line.strip()
            if not line or line.startswith('#'):
                if '# Данные' in line:
                    section = 'data'
                continue
            
            if section == 'data':
                parts = line.split()
                if len(parts) == 2:
                    data.append(float(parts[0]))
                    censored.append(int(parts[1]))
            else:
                parts = line.split()
                if len(parts) == 2:
                    params[parts[0]] = float(parts[1])
    
    return params, np.array(data), np.array(censored)

def plot_mle_normal(params, data, censored, output_file):
    """Построение графика на вероятностной бумаге для MLE нормального распределения"""
    mean = params['parameter_1']
    std = params['parameter_2']

    # Увеличиваем размер для лучшего качества при масштабировании
    fig, ax = plt.subplots(figsize=(14, 9), dpi=100)
    fig.suptitle('Normal MLE', fontsize=14, fontweight='bold')

    complete_data = data[censored == 0]
    n = len(complete_data)

    # Сортируем данные для получения порядковых статистик
    sorted_data = np.sort(complete_data)

    # Вычисляем эмпирические вероятности (i - 0.375) / (n + 0.25)
    i = np.arange(1, n + 1)
    empirical_probs = (i - 0.375) / (n + 0.25)

    # Квантили стандартного нормального распределения
    z_empirical = stats.norm.ppf(empirical_probs)

    # 1. Точки данных (эмпирические)
    ax.scatter(sorted_data, z_empirical,
              color='darkblue', s=80, alpha=0.8,
              edgecolors='black', linewidths=1,
              label='Data points', zorder=3)

    # 2. Апроксимирующая прямая (теоретическое распределение)
    # z = (x - μ) / σ
    # Ограничиваем диапазон только данными
    x_line = np.linspace(sorted_data.min(), sorted_data.max(), 200)
    z_line = (x_line - mean) / std
    ax.plot(x_line, z_line, 'r-', linewidth=2.5,
            label='Fitted line', zorder=2)

    # 3. Доверительные границы (отклонения) - плавные дуги
    # Вычисляем доверительные границы вдоль теоретической линии
    # Ограничиваем область построения, чтобы избежать артефактов на краях

    # Теоретические вероятности для x_line
    probs_line = stats.norm.cdf(x_line, mean, std)

    # Фильтруем только разумный диапазон вероятностей (избегаем краев)
    valid_mask = (probs_line > 0.01) & (probs_line < 0.99)
    x_conf = x_line[valid_mask]
    probs_conf = probs_line[valid_mask]

    # Стандартная ошибка для вероятности с поправкой на квантили
    # Используем дельта-метод: SE(Φ⁻¹(F)) ≈ SE(F) / φ(Φ⁻¹(F))
    z_conf = stats.norm.ppf(probs_conf)
    phi_z = stats.norm.pdf(z_conf)  # плотность в точке квантиля

    # SE для эмпирической функции распределения
    se_prob = np.sqrt(probs_conf * (1 - probs_conf) / n)

    # SE для квантилей (дельта-метод)
    se_quantile = se_prob / phi_z

    confidence_level = 1.96  # 95% доверительный интервал

    # Доверительные интервалы для квантилей
    z_upper = z_conf + confidence_level * se_quantile
    z_lower = z_conf - confidence_level * se_quantile

    # Верхняя граница (плавная дуга)
    ax.plot(x_conf, z_upper, 'b--', linewidth=2,
            alpha=0.7, label='Confidence bounds', zorder=1)

    # Нижняя граница (плавная дуга)
    ax.plot(x_conf, z_lower, 'b--', linewidth=2,
            alpha=0.7, zorder=1)

    ax.set_xlabel('x', fontsize=12)
    ax.set_ylabel('Quantiles (standard normal)', fontsize=12)
    ax.legend(fontsize=11, loc='best')
    ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.8)
    ax.grid(True, which='minor', alpha=0.15, linestyle=':', linewidth=0.5)
    ax.minorticks_on()

    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"График сохранен: {output_file}")
    plt.close()

def plot_mls_normal(params, data, censored, output_file):
    """Построение графика на вероятностной бумаге для MLS нормального распределения"""
    mean = params['parameter_1']
    std = params['parameter_2']

    # Увеличиваем размер для лучшего качества при масштабировании
    fig, ax = plt.subplots(figsize=(14, 9), dpi=100)
    fig.suptitle('Normal MLS', fontsize=14, fontweight='bold')

    complete_data = data[censored == 0]
    censored_data = data[censored == 1]
    n = len(complete_data)

    # Сортируем данные для получения порядковых статистик
    sorted_data = np.sort(complete_data)

    # Вычисляем эмпирические вероятности (i - 0.375) / (n + 0.25)
    i = np.arange(1, n + 1)
    empirical_probs = (i - 0.375) / (n + 0.25)

    # Квантили стандартного нормального распределения
    z_empirical = stats.norm.ppf(empirical_probs)

    # 1. Точки данных (эмпирические - полные наблюдения)
    ax.scatter(sorted_data, z_empirical,
              color='darkblue', s=80, alpha=0.8,
              edgecolors='black', linewidths=1,
              label='Complete data', zorder=3)

    # Цензурированные данные - показываем крестиками
    if len(censored_data) > 0:
        # Для цензурированных данных используем теоретическую линию
        z_censored = (censored_data - mean) / std
        ax.scatter(censored_data, z_censored,
                  marker='x', color='red', s=100, linewidths=2,
                  label='Censored data', zorder=3)

    # 2. Апроксимирующая прямая (теоретическое распределение)
    # z = (x - μ) / σ
    # Ограничиваем диапазон только полными данными
    x_line = np.linspace(sorted_data.min(), sorted_data.max(), 200)
    z_line = (x_line - mean) / std
    ax.plot(x_line, z_line, 'r-', linewidth=2.5,
            label='Fitted line', zorder=2)

    # 3. Доверительные границы (отклонения) - плавные дуги
    # Вычисляем доверительные границы вдоль теоретической линии
    # Ограничиваем область построения, чтобы избежать артефактов на краях

    # Теоретические вероятности для x_line
    probs_line = stats.norm.cdf(x_line, mean, std)

    # Фильтруем только разумный диапазон вероятностей (избегаем краев)
    valid_mask = (probs_line > 0.01) & (probs_line < 0.99)
    x_conf = x_line[valid_mask]
    probs_conf = probs_line[valid_mask]

    # Стандартная ошибка для вероятности с поправкой на квантили
    # Используем дельта-метод: SE(Φ⁻¹(F)) ≈ SE(F) / φ(Φ⁻¹(F))
    z_conf = stats.norm.ppf(probs_conf)
    phi_z = stats.norm.pdf(z_conf)  # плотность в точке квантиля

    # SE для эмпирической функции распределения
    se_prob = np.sqrt(probs_conf * (1 - probs_conf) / n)

    # SE для квантилей (дельта-метод)
    se_quantile = se_prob / phi_z

    confidence_level = 1.96  # 95% доверительный интервал

    # Доверительные интервалы для квантилей
    z_upper = z_conf + confidence_level * se_quantile
    z_lower = z_conf - confidence_level * se_quantile

    # Верхняя граница (плавная дуга)
    ax.plot(x_conf, z_upper, 'b--', linewidth=2,
            alpha=0.7, label='Confidence bounds', zorder=1)

    # Нижняя граница (плавная дуга)
    ax.plot(x_conf, z_lower, 'b--', linewidth=2,
            alpha=0.7, zorder=1)

    ax.set_xlabel('x', fontsize=12)
    ax.set_ylabel('Quantiles (standard normal)', fontsize=12)
    ax.legend(fontsize=11, loc='best')
    ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.8)
    ax.grid(True, which='minor', alpha=0.15, linestyle=':', linewidth=0.5)
    ax.minorticks_on()

    plt.tight_layout()
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    print(f"График сохранен: {output_file}")
    plt.close()

def main():
    if len(sys.argv) < 2:
        print("Использование: python plot_normal.py [mle|mls]")
        sys.exit(1)
    
    mode = sys.argv[1].lower()

    # Используем абсолютные пути относительно текущей директории (которая должна быть /alg2)
    if mode == 'mle':
        results_file = 'output/mle_normal_complete.txt'
        output_file = 'output/plot_mle_normal.png'
        params, data, censored = read_results(results_file)
        plot_mle_normal(params, data, censored, output_file)
    elif mode == 'mls':
        results_file = 'output/mls_normal_complete.txt'
        output_file = 'output/plot_mls_normal.png'
        params, data, censored = read_results(results_file)
        plot_mls_normal(params, data, censored, output_file)
    else:
        print("Ошибка: укажите 'mle' или 'mls'")
        sys.exit(1)

if __name__ == '__main__':
    main()